...
from job_hub.serializers import CategorySerializer
...
def everyday_calculation_of_added_vacancies(self):
    ...
    print(f"Number of new vacancies during the last day: {added_vacancies_during_last_day}")
    send_message(f"Number of new vacancies during the last day: {added_vacancies_during_last_day}")
...
def update_google_sheet(self, vacancy_id):
    ...
    vacancy = Vacancy.objects.select_related("category", "salary", "company_profile").prefetch_related(
        "associated_locations").get(id=vacancy_id)
    ...
    vacancy_data.append([
        ...
        ", ".join(location.location_name for location in vacancy.associated_locations.all()),
        str(vacancy.salary),
